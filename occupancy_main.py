...
if __name__ == '__main__':
    # load a scene
    dataroot = 'D:\SEM-3\AV\Lab_2\data'
    ...
    for i in tqdm(range(scene['nbr_samples'])):
        ...
        if i == 0:
            ...
            f0_M_i = np.linalg.inv(w_M_0) @ w_M_i  # TODO: note to inverse a matrix use np.linalg.inv
        ...
        if i > 0:
            points_3d = homogenize(points_3d)
            points_3d = (f0_M_i @ points_3d.T).T  # TODO
            #print(points_3d)
            #print(points_3d.shape)

        vehicle_xy = np.zeros(2) if i == 0 else f0_M_i[:2, -1]  # TODO: get xy position of i-th ego frame in the first ego frame
